`ifndef PIPE_CONTROL_V
`define PIPE_CONTROL_V
`timescale 1ns/1ps
/*
Pipeline Control Unit for 5-Stage Pipelined RISC-V Processor
Generates control signals from opcode, funct3, funct7.
Signals are generated in the ID stage and propagated through
pipeline registers to subsequent stages.
*/

module PIPE_CONTROL(
    input [6:0] opcode,
    input [2:0] funct3,
    input [6:0] funct7,
    output reg [3:0] alu_control,
    output reg regwrite,
    output reg alu_src,         // 0=register, 1=immediate
    output reg mem_read,
    output reg mem_write,
    output reg mem_to_reg,      // 0=ALU result, 1=memory data
    output reg branch,
    output reg jump,
    output reg alu_pc_src       // 1=use PC as ALU input1 (for AUIPC)
);

    // Opcode definitions
    localparam R_TYPE  = 7'b0110011;
    localparam I_TYPE  = 7'b0010011;
    localparam LOAD    = 7'b0000011;
    localparam STORE   = 7'b0100011;
    localparam BRANCH  = 7'b1100011;
    localparam JAL     = 7'b1101111;
    localparam JALR    = 7'b1100111;
    localparam LUI     = 7'b0110111;
    localparam AUIPC   = 7'b0010111;

    always @(*) begin
        // Default values — all disabled
        regwrite    = 1'b0;
        alu_src     = 1'b0;
        mem_read    = 1'b0;
        mem_write   = 1'b0;
        mem_to_reg  = 1'b0;
        branch      = 1'b0;
        jump        = 1'b0;
        alu_pc_src  = 1'b0;
        alu_control = 4'b1111;
        
        case(opcode)
            R_TYPE: begin
                regwrite = 1'b1;
                alu_src  = 1'b0;
                case({funct7, funct3})
                    {7'b0000000, 3'b000}: alu_control = 4'b0010; // ADD
                    {7'b0100000, 3'b000}: alu_control = 4'b0100; // SUB
                    {7'b0000000, 3'b001}: alu_control = 4'b0011; // SLL
                    {7'b0000000, 3'b010}: alu_control = 4'b1000; // SLT
                    {7'b0000000, 3'b011}: alu_control = 4'b1001; // SLTU
                    {7'b0000000, 3'b100}: alu_control = 4'b0111; // XOR
                    {7'b0000000, 3'b101}: alu_control = 4'b0101; // SRL
                    {7'b0100000, 3'b101}: alu_control = 4'b1010; // SRA
                    {7'b0000000, 3'b110}: alu_control = 4'b0001; // OR
                    {7'b0000000, 3'b111}: alu_control = 4'b0000; // AND
                    {7'b0000001, 3'b000}: alu_control = 4'b0110; // MUL
                    default: alu_control = 4'b1111;
                endcase
            end
            
            I_TYPE: begin
                regwrite = 1'b1;
                alu_src  = 1'b1;
                case(funct3)
                    3'b000: alu_control = 4'b0010; // ADDI
                    3'b010: alu_control = 4'b1000; // SLTI
                    3'b011: alu_control = 4'b1001; // SLTIU
                    3'b100: alu_control = 4'b0111; // XORI
                    3'b110: alu_control = 4'b0001; // ORI
                    3'b111: alu_control = 4'b0000; // ANDI
                    3'b001: alu_control = 4'b0011; // SLLI
                    3'b101: begin
                        if (funct7 == 7'b0000000)
                            alu_control = 4'b0101; // SRLI
                        else
                            alu_control = 4'b1010; // SRAI
                    end
                    default: alu_control = 4'b1111;
                endcase
            end
            
            LOAD: begin
                regwrite    = 1'b1;
                alu_src     = 1'b1;
                mem_read    = 1'b1;
                mem_to_reg  = 1'b1;
                alu_control = 4'b0010; // ADD for address
            end
            
            STORE: begin
                alu_src     = 1'b1;
                mem_write   = 1'b1;
                alu_control = 4'b0010; // ADD for address
            end
            
            BRANCH: begin
                alu_src = 1'b0;
                branch  = 1'b1;
                case(funct3)
                    3'b000: alu_control = 4'b0100; // BEQ (SUB)
                    3'b001: alu_control = 4'b0100; // BNE (SUB)
                    3'b100: alu_control = 4'b1000; // BLT (SLT)
                    3'b101: alu_control = 4'b1000; // BGE (SLT)
                    3'b110: alu_control = 4'b1001; // BLTU (SLTU)
                    3'b111: alu_control = 4'b1001; // BGEU (SLTU)
                    default: alu_control = 4'b1111;
                endcase
            end
            
            JAL: begin
                regwrite    = 1'b1;
                jump        = 1'b1;
                alu_control = 4'b0010; // ADD (unused — PC+4 written back)
            end

            // BUG-3 FIX: JALR needs alu_src=1 so ALU computes rs1+imm correctly
            JALR: begin
                regwrite    = 1'b1;
                jump        = 1'b1;
                alu_src     = 1'b1;    // Select immediate for ALU input 2
                alu_control = 4'b0010; // ADD (rs1 + imm for target calc)
            end
            
            LUI: begin
                regwrite    = 1'b1;
                alu_src     = 1'b1;
                alu_control = 4'b1011; // Pass immediate
            end
            
            AUIPC: begin
                regwrite    = 1'b1;
                alu_src     = 1'b1;
                alu_pc_src  = 1'b1;
                alu_control = 4'b0010; // ADD (PC + immediate)
            end
            
            default: begin
                // All outputs stay at default (NOP behavior)
            end
        endcase
    end
endmodule
`endif
